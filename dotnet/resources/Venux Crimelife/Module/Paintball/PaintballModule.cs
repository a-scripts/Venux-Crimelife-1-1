...
namespace Venux
{
    class PaintballModule : Venux.Module.Module<PaintballModule>
    {
            ...
            try
            {
                ...
                foreach (var t in Zones)
                {
                    if (t.Players().Count >= 10)
                    {
                        ...
                        dbPlayer.SendNotification("Die FFA/Paintball Arena ist bereits voll!", 3000, "red");
                        return;
                    }
                }
                ...
                if (zone != null)
                {
                    ...
                    Random r = new Random();
                    c.Dimension = Convert.ToUInt32(22750);
        ...
        public static void PaintballDeath(DbPlayer dbPlayer, DbPlayer dbPlayer2)
        {
            try
            {
                ...
                if (killstreak == 3)
                {
                    paintballModel.Players().ForEach(p => Notification.SendGlobalNotification(p.Client, "Bei " + dbPlayer2.Name + " läuft es gerade GUT!", 5000, "white", Notification.icon.bullhorn));
                }

                else if (killstreak == 5)
                {
                    paintballModel.Players().ForEach(p => Notification.SendGlobalNotification(p.Client, dbPlayer2.Name + " ist richtig dabei!", 5000, "white", Notification.icon.bullhorn));
                }
                ...
                {
                    paintballModel.Players().ForEach(p => Notification.SendGlobalNotification(p.Client, dbPlayer2.Name + " ist nicht aufzuhalten!", 5000, "white", Notification.icon.bullhorn));
                }
