...
namespace Venux
{
    class VoiceModule : Module<VoiceModule>
    {
                ...
                if (int.TryParse(val2, out val))
                {
                    ...
                    if ((player == null || !player.IsValid(true) ? false : player.Client != null))
                    {
                        if (player.CanInteractAntiDeath())
                        {
                            ...
                            {
                                if ((dynamic)c.GetSharedData<int>("FUNK_CHANNEL") != 0)
                                {
                                    c.SetSharedData("FUNK_TALKING", false);
                                    if (!c.IsInVehicle)
                                    {
        ...
        public static void changeFrequenz(Player c, string val2)
        {
            try
            {
                ...
                Faction faction = FactionModule.factionList.Find((Faction faction2) => faction2.Id == val);
                if (faction != null)
                {
                ...
                if (encrypted)
                {
                    dbPlayer.SendNotification("Dieser Funkkanal ist verschlüsselt. ( "+val2+" )", 3000, "red");
                    return;
                }
