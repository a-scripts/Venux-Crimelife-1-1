...
namespace Venux
{
   public class DonatorShopModule : Module<DonatorShopModule>
    {
                    ...
                    if ((player == null || !player.IsValid(true) ? false : player.Client != null))
                    {
                        if (player.CanInteractAntiFlood(1))
                        {
                            if (selection != null)
                            {
                                ...
                                {
                                    player.SendNotification("Du hast dafür kein Zugriff! (Kein Donator)", 3000, "red", "");
                                }
                                else
                                {
                                    try
        ...
        {
            Vector3 vector3 = new Vector3(-266.32, -961.72, 30.30);
                ...
                if (client != null)
                {
                    ...
                    if ((player == null || !player.IsValid(true) ? false : player.Client != null))
                    {
                        if (player.CanInteractAntiFlood(1))
                        {
                            if ((dynamic)player.GetAttributeInt("Donator") != 1)
                            {
                                player.SendNotification("Dazu hast du kein Zugriff! (Du bist kein Donator)", 3000, "red", "");
                            }
                            else
                            {
                                List<NativeItem> list = new List<NativeItem>();
