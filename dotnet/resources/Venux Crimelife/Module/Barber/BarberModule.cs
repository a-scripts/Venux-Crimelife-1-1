...
namespace Venux
{
    class BarberModule : Module<BarberModule>
    {
        ...
        public void barberShopBuy(Player c, int price, string json)
        {
            try
            {
                ...
                if (dbPlayer.Money <= price)
                {
                    ...
                    List<Decoration> tattoos1 = NAPI.Util.FromJson<List<Decoration>>(dbPlayer.GetAttributeString("Tattoo"));
                    foreach (Decoration tattooid in tattoos1)
                    {
