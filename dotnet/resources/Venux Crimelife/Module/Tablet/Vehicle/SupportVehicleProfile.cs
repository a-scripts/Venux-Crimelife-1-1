                        ...
                        while ((reader).Read())
                        {
                            ...
                            foreach (GarageVehicle garageVehicles in MySqlManager.GetAllVehicles())
                            {
                                ...
                                if (garageVehicles.Id == primary)
                                {
                                    ...
                                    if (primary == 1) return;
                                    p.TriggerEvent("componentServerEvent", new object[3]{"SupportVehicleProfile","responseVehicleData","[{\"id\":\"" + garageVehicles.Id +"\",\"vehiclehash\":\"" + garageVehicles.Name +"\",\"inGarage\":\"" + parked +"\",\"owner\":\"" + reader2.GetString("Username") +"\",\"garage\":\"" + garageVehicles.Garage +"\"}]"
                                    });
                                    //});
