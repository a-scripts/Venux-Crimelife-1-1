...
namespace Venux
{
    public static class MySqlManager
    {
		...
		public static List<ItemModel> GetHandschuhItems(this DbVehicle dbVehicle)
		{
			//IL_000d: Unknown result type (might be due to invalid IL or missing references)
			//IL_0013: Expected O, but got Unknown
			List<ItemModel> list = new List<ItemModel>();
					...
					try
					{
						...
						try
						{
							if (((DbDataReader)(object)val3).HasRows)
							{
								while (((DbDataReader)(object)val3).Read())
								{
									list = NAPI.Util.FromJson<List<ItemModel>>(val3.GetString("Handschuh"));
									int Slot = 0;
									list.ForEach(delegate (ItemModel item)
									{
										item.Slot = Slot;
		...
		public static List<ItemModel> GetRobItems(this DbPlayer dbPlayer)
		{
			//IL_000d: Unknown result type (might be due to invalid IL or missing references)
			//IL_0013: Expected O, but got Unknown
			List<ItemModel> list = new List<ItemModel>();
			try
			{
				...
				try
				{
					try
					{
						((DbConnection)(object)val).Open();
						MySqlCommand val2 = val.CreateCommand();
						((DbCommand)(object)val2).CommandText = "SELECT * FROM rob";
						MySqlDataReader val3 = val2.ExecuteReader();
						try
						{
							if (!((DbDataReader)(object)val3).HasRows)
							{
								val3.Dispose();
								MySqlCommand val4 = val.CreateCommand();
								((DbCommand)(object)val4).CommandText = "INSERT INTO `rob` (`Items`) VALUES(@inv)";
								val4.Parameters.AddWithValue("@inv", (object)"[]");
								((DbCommand)(object)val4).ExecuteNonQueryAsync();
							}
							else
			...
			{
				Logger.Print("[EXCEPTION GetRobItems] " + ex.Message);
				Logger.Print("[EXCEPTION GetRobItems] " + ex.StackTrace);
			}
			return list;
		}
