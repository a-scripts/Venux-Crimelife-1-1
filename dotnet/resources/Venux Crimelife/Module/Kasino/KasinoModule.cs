...
namespace Venux.Module.Kasino
{
    public class KasinoModule : Module.Module<KasinoModule>
    {
		...
		public List<KasinoDevice> KasinoDevices = new List<KasinoDevice>();
		...
		private float calculateProfit(int slot1, int slot2, int slot3, int einsatz)
		{
			...
			return num * (float)einsatz;
		}

		public KasinoDevice GetClosest(DbPlayer dbPlayer)
		{
			return KasinoDevices.FirstOrDefault(kasinoDevice => kasinoDevice.Position.DistanceTo(((Entity)dbPlayer.Client).Position) < (float)kasinoDevice.Radius);
		}
