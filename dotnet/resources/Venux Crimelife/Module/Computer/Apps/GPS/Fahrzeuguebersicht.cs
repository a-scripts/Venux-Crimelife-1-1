...
namespace Venux
{
	public class FahrzeugÜbersichtApp : Script
	{
		...
		public void requestVehicleOverviewByCategory(Player p, int id)
		{
			...
			List<OwnVehicleModel> businesskeys = new List<OwnVehicleModel>();
			...
			foreach (GarageVehicle garageVehicles in MySqlManager.GetAllVehicles())
			{
				...
				if (garageVehicles.Keys.Contains(dbPlayer.Business.Id))
					businesskeys.Add(new OwnVehicleModel(garageVehicles.Name, garageVehicles.Id, garageVehicles.Parked, garageVehicles.Garage, carCor));
			}
