...
namespace Venux
{
    class GangwarModule : Module<GangwarModule>
    {
        ...
        {
            return GWZones.Find((Gangwar GWZone) => GWZone.Id == Id);
        }
        ...
        public void Gangwar(Player c, string selection)
        {
            try
            {
                ...
                foreach (DbPlayer dbTarget in gw.Faction.GetFactionPlayers())
                {
                        ...
                        dbTarget.SendNotification($"Das Gebiet {gw.Name} wird von der Fraktion {gw.Attacker.Name} angegriffen.", 3000, "orange", "GANGWAR");
                }
                foreach (DbPlayer dbTarget in gw.Attacker.GetFactionPlayers())
                {
                    if (dbTarget != null && dbTarget.IsValid(true))
                        dbTarget.SendNotification($"Deine Fraktion greift das Gebiet {gw.Name} an.", 3000, "orange", "GANGWAR");
                }
                Notification.SendGlobalNotification($"Die Fraktion {gw.Attacker.Name} greift das Gebiet {gw.Name} von der Fraktion {gw.Faction.Name} an.", 8000, "white", Notification.icon.warn);
                ...
                Marker m = NAPI.Marker.CreateMarker(1, gw.Zone, new Vector3(), new Vector3(), 300, new Color(155, 155, 0), false, Convert.ToUInt32(FactionModule.GangwarDimension));
        ...
        public static void handleKill(DbPlayer killer)
        {
            try
            {
                ...
                killer.Faction.GetFactionPlayers()
                    .ForEach((DbPlayer dbPlayer) => dbPlayer.SendNotification("+3 Punkte für das töten eines Gegners (" + killer.Name + ")."));
        ...
        public static void EndGangwar()
        {
            try
            {
                ...
                foreach (DbPlayer player in PlayerHandler.GetPlayers())
                {
                    ...
                    player.SetPosition(player.Faction.Spawn);
                }
            }
            catch (Exception ex)
            {
