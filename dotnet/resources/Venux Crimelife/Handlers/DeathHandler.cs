...
namespace Venux
{
    internal class DeathHandler : Script
    {
        ...
        public void 
            eath(Player c, Player k, uint reason)
        {
            try
            {
                if (c == null) return;
                ...
                if (paintballModel != null)
                {
                    if (k != null && k.Exists)
                    {
                        ...
                        dbPlayer.SendNotification("Du wurdest von " + dbKiller.Name + " getötet!");
                        dbKiller.SendNotification("Du hast " + dbPlayer.Name + " getötet! +30.000$");
                ...
                if (dbPlayer.HasData("IN_GANGWAR"))
                {
                    ...
                    if (k != null && k.Exists)
                    {
                        ...
                        dbPlayer.SendNotification("Du wurdest von " + dbKiller.Name + " getötet!", 5000);
                        dbKiller.SendNotification("Du hast " + dbPlayer.Name + " getötet! +30.000$");
                        dbKiller.addMoney(30000);
                    }
                    else
                    {
                        dbPlayer.SendNotification("Du bist gestorben!", 3000);
                    }
                ...
                else if (paintballModel == null)
                {
                    ...
                    //dbPlayer.TriggerEvent("openWindow", new object[] { "Death" });
